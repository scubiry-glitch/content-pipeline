...
class AutoTagService {
  ...
  extractTagsFromContent(content: string): string[] {
    ...
    // 1. 提取高频词（排除停用词）- BUG-302-Next FIX
    const words = content.split(/[\s,，。！？；：""''（）【】]+/);
    const wordCount = new Map<string, number>();
    const totalWords = words.length;

    words.forEach(word => {
      // 过滤条件：长度4-10，不含数字，不是停用词 - BUG-302-Next FIX
    ...
    // 2. 计算TF-IDF-like分数（频率 * 独特性）- BUG-302-Next FIX
    ...
    // 3. 提取关键短语（2-3个词的组合）- BUG-302-Next FIX
