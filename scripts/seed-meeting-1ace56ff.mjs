// Seed · 上海汇聚 AI 项目启动会 (匿名版 · 成员甲/成员乙)
// 来源：/Users/scubiry/Downloads/sh-ai-kickoff-2026-03-31__analysis (2).ts
// 目标：assets.id = 1ace56ff-1eeb-48d9-8f6b-e3070d9338a4
//
// 用法：node scripts/seed-meeting-1ace56ff.mjs
...
const MEETING_ID = '1ace56ff-1eeb-48d9-8f6b-e3070d9338a4';
...
const PARTICIPANTS = [
  ...
  { id: 'p3', name: '成员甲',   role: '项目执行/协调 · 转写未署名 (说话人 3)',    initials: '甲', tone: 'neutral', speakingPct: 8  },
  { id: 'p4', name: '成员乙',   role: '运营/服务方向 · 转写未署名 (说话人 4)',    initials: '乙', tone: 'warm',    speakingPct: 12 },
];
...
const ANALYSIS = {
  summary: {
    decision:
      ...
      '核心团队从「永邦 + 一濛 + 成员甲」三人组扩为加入成员乙的四人组。' +
      '6 个月内拿到 2-3 个可对外讲的 AI 提效成果, 把上海汇聚从「AI 应用基本为 0」推到全国头部。',
    actionItems: [
      { id: 'A1', who: 'p2', what: '与永邦共同「写剧本」: 把房源评价 / 资管经理末端行为 / 租户管家看板等场景拆成具体 B 点画面', due: '1 个月窗口 (本场后 30 天)' },
      { id: 'A2', who: 'p2', what: '搭桥数据源: 三毛 / 人力 / 法务 / 财务 跨部门数据接入',                                       due: '与剧本并行' },
      { id: 'A3', who: 'p1', what: '把「管一套房 / 管一个人 / 管每个指标」三个画面收敛为可对外讲的 demo',                       due: '半年节点' },
    ...
    risks: [
      'R1 · 业务非原子化 — 成员乙明确指出「我们今天业务场景当中有很多交叉和交互配合的」, 不能直接照搬美团/滴滴的末端激励模型',
      'R2 · 跨部门数据源对接难度大 (三毛/人力/法务/财务), 是项目最关键的一道坎, 不是「代码值不值钱」能解决的',
      'R3 · 「颠覆式」vs「提效式」路线尚未对齐 — 永邦明确要颠覆, 成员乙暗示部分场景应保留传统管理',
      'R4 · 三人/四人组规模过小, 而上海汇聚整体「AI 应用基本上是 0」, 团队带宽不足以同时撑起场景定义 + 数据对接 + demo 落地',
      'R5 · 永邦自评目标"太宏观" (成员乙原话:「你这个想要的稍微有点太宏观啊」), 若不在月内收敛到具体场景, 项目会停留在意愿层',
  ...
  tension: [
    {
      ...
      summary:
        ...
        ' 成员乙指出供应链、商机分配等环节是「交叉和交互配合」的协作场景,' +
        ' 「我很努力, 我供应商, 他签单我还需要激励吗?」 — 个人努力无法直接兑现, 美团模型只是参考、不是模板。' +
        ' 永邦回应「有些东西你该传统管理」, 部分接受了对方的边界。',
      moments: [
        '永邦: 「我要把所有推翻。反正我觉得现在不太, 这个墙也没修多高。」',
        '成员乙: 「今天业务场景当中有很多交叉和交互配合的…他是一种和别人是在交互合作的场景下。」',
    ...
    },
    {
      ...
      summary:
        ...
        ' 成员乙 + 一濛 共同把球拉回到「画面感 / 维度 / 具体场景」 —' +
        ' 成员乙直白「你这个想要的稍微有点太宏观」, 一濛补「demo 出来你看是不是你想要的」。' +
        ' 永邦最终把"两个阶段、第一阶段写剧本"作为收敛动作。',
      moments: [
        '成员乙: 「其实最缺的是画面感。就你到底想实现什么?」',
    ...
    },
    {
      ...
      summary:
        ...
        ' 成员甲反复追问"一阶段 → 二阶段 → 管理者怎么管管理者"的执行路径与验证逻辑。' +
        ' 这条张力低烈度, 但代表了"愿景驱动 vs 工程驱动"两种节奏的拉扯。',
      moments: [
        '成员甲: 「假如说我们出来一阶段, 这个东西出来之后, 那个一线的人我们知道他在干嘛, 所有的都有, 然后最终二阶段实现的就是…接下来的管理。」',
    ...
    },
    {
      ...
      summary:
        ...
        ' 二人没有正面对抗 — 一濛只是温和示范"代码不值钱、想法值钱", 永邦当场被打动 (见 N1)。',
      moments: [
  ...
  newCognition: [
    {
      ...
      trigger: '一濛现场展示装修评分 demo + 透露"其实一天"做完, 加上成员乙点出"最缺的是画面感"。',
    },
    {
      id: 'N2', who: 'p1',
    ...
    },
    {
      ...
      trigger: '在永邦反复举美团例后, 成员乙主动指出业务非原子化的边界。',
    },
    {
      id: 'N8', who: 'p1',
      before: '设计师会下岗 (AI 替代)',
      after: '设计师不会下岗, 而是转销售/转型 — 工具升级带来的是角色重塑',
      trigger: '成员乙的细化 (「这是设计师的工具, 下岗总要有个人说」) + 一濛补刀 (「销售就是设计师没了, 变成销售了」)。',
    },
  ],
  focusMap: [
  ...
  consensus: [
    ...
    { id: 'C3', kind: 'consensus',  text: '团队从三人组 (永邦 + 一濛 + 成员甲) 扩展为加入成员乙的四人组',                  supportedBy: ['p1','p4'],           sides: [] },
...
try {
  ...
  if (exists.rowCount === 0) {
    ...
    console.log(`[seed] INSERTed ${MEETING_ID}`);
  } else {
    await client.query(
      `UPDATE assets
          SET title = $2,
    ...
    console.log(`[seed] UPDATEd ${MEETING_ID}`);
  }
