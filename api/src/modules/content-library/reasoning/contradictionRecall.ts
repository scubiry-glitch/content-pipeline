...
export interface TensionCandidate extends Contradiction {
  ...
  timeSlice?: string;
  /** 背后的心智模型冲突，例如"效率 vs 公平"、"技术乐观主义 vs 技术恐慌" */
...
const L3_SYSTEM_PROMPT = `你是矛盾分类专家。对每条矛盾候选,判断它属于以下 5 种张力类型中的哪一种,并抽取 divergence_axis / parties / time_slice / underlying_model。
...
字段说明:
- divergenceAxis: 分歧所在的具体维度（如"增长方式"、"社会成本"）
- parties: 对立双方各自的立场（name 可为机构/观点流派/群体）
- timeSlice: 事实 A 与事实 B 的时间背景差异
- underlyingModel: 双方背后更深层的心智模型冲突，用"X vs Y"格式表达（如"技术乐观主义 vs 社会风险论"、"效率优先 vs 公平优先"）
...
[
  {
    ...
    "underlyingModel": "心智模型 A vs 心智模型 B"
  }
]`;

const L3_BATCH_SIZE = 10; // 每批最多 10 条，避免单次 prompt 过长
...
async function classifyL3(
  ...
  // 分批处理，每批独立调用 LLM，结果按原始顺序合并
  const results: TensionCandidate[] = new Array(candidates.length);
  for (let batchStart = 0; batchStart < candidates.length; batchStart += L3_BATCH_SIZE) {
    const batch = candidates.slice(batchStart, batchStart + L3_BATCH_SIZE);
    const classified = await classifyL3Batch(deps, batch, batchStart);
    for (let i = 0; i < classified.length; i++) {
      results[batchStart + i] = classified[i];
    }
  }
  return results;
}

async function classifyL3Batch(
  deps: ContentLibraryDeps,
  batch: TensionCandidate[],
  indexOffset: number,
): Promise<TensionCandidate[]> {
  const userPrompt = buildL3Prompt(batch, indexOffset);
  try {
    ...
    if (!Array.isArray(parsed)) {
      console.warn('[contradictionRecall] L3 batch parse failed, raw:', raw?.slice(0, 200));
      return batch.map(c => ({ ...c, tensionType: 'unknown' as TensionType }));
    }

    return batch.map((c, localIdx) => {
      const globalIdx = indexOffset + localIdx;
      const match = parsed.find((p: any) => p?.index === globalIdx) ?? parsed[localIdx];
      ...
      return {
        ...
        underlyingModel: typeof match.underlyingModel === 'string' ? match.underlyingModel : undefined,
        recallLayer: 'L3',
      };
    });
  } catch (err) {
    console.warn('[contradictionRecall] L3 batch classify failed:', (err as Error).message);
    return batch.map(c => ({ ...c, tensionType: 'unknown' as TensionType }));
  }
}

function buildL3Prompt(candidates: TensionCandidate[], indexOffset = 0): string {
  const lines = candidates.map((c, localIdx) => {
    const globalIdx = indexOffset + localIdx;
    ...
    return `[${globalIdx}] 事实A(${timeA}): ${factA}\n    事实B(${timeB}): ${factB}`;
  });
  return `请对以下 ${candidates.length} 条矛盾候选逐条分类并抽取 divergence_axis / parties / time_slice / underlying_model:\n\n${lines.join('\n\n')}\n\n按索引输出 JSON 数组。`;
}
