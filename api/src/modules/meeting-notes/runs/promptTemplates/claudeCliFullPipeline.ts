...
wikiMarkdown: 给 data/content-wiki/default/ 直接写文件用的 markdown 内容。
  形态:
  {
    sourceEntry: string,    // sources/<meetingId>.md 全文 markdown, 见下方 frontmatter 模板
    entityUpdates: Array<{
      entityName: string,   // 必须用会议里明确出现的人名/概念名 (后端会先查 content_entities 命中才追加)
      appendMarkdown: string  // 追加到 entities/<entityName>.md 末尾的内容
    }>
  }

  sourceEntry 模板 (字符串里需包含):
  ---
  meetingId: <id>
  date: <date>
  title: <title>
  ...
  ## Wiki 实体引用
  - [[<entity1>]] · 在 fact #<n> 中提及
  - ...

  约束:
    - sourceEntry 必须以 frontmatter 三段杠开头, 否则 obsidian 解析不正确
    - participants frontmatter 用 name 数组, 不用 localId
    - entityUpdates 仅对**确定已出现**的实体 (参与者人名 + 转写明确名词) 输出, 避免脏数据
    - entityUpdates[].appendMarkdown 不必再加 ## title, 后端追加时自动加 \\n## Claude CLI · meeting <id>\\n 段落标题`);
