...
const AXES_SCHEMA_SPEC = `
{
  ...
  meta: {
    ...
    emotionCurve: [
      ...
      // 数量下限: ≥10 条 (会议 <5min 时按 3/min, 即 5min 应有 ≥15 条;
      //                  覆盖关键转折点: 开场/议题切换/分歧/共识/结束 + 中间均匀采样)
      //
      // v 必须是【连续值】, 禁止把每个张力期采样点都打成 -1 或每个共识点都打成 1.
      // 经验分布(强约束):
      //   - 中性 (|v|<0.3): 占 50%+ 的采样, 大多数汇报/讨论/共识段
      //   - 较强 (0.3 ≤ |v| < 0.7): 占 ~30%, 有明显倾向但未爆发的争论 / 兴奋
      //   - 极端 (|v| ≥ 0.85): 整场会议 ≤2 个采样点, 仅留给最爆发的瞬间
      // 反例(禁止): [v=-1, v=-1, v=-1, v=-1, ...] 连续多个相同极值
      // 正例: [v=0.1, v=-0.4, v=-0.85, v=-0.6, v=-0.3, v=0, v=0.2] 连续渐变
    ]
  },
...
1. 严格 JSON, UTF-8, 能被 JSON.parse 直接吃。不要任何 markdown 代码块。
   1a. 【JSON 字符串转义铁律】字符串值内只要出现 ASCII 双引号 \`"\`, 必须转义为 \`\\"\`。
       否则解析器会把字符串过早闭合, 整 run 失败 (历史 case: run 988147ac, 一处未转义 \`"\` 把 38KB 输出全废)。
   1b. 【推荐替代】中文叙述里需要"引用短语"时, 优先用「」/『』/单引号 ' ' 包裹, 不要用 ASCII \`"\`。
       例:  ✓ 改以「特征值输出+应用包装」的形式
            ✗ 改以"特征值输出+应用包装"的形式   ← ASCII 双引号嵌中文里, 必须 \\" 转义
   1c. 【括号铁律】对象 \`{}\` / 数组 \`[]\` 严格配对闭合, 不要多打或漏打 \`}\` / \`]\`。
       特别是数组结尾: 写完最后一个元素就 \`]\`, 不要在 \`]\` 后再补 \`}\` 收尾外层 (除非外层确实是对象)。
   1d. 输出前心算一次每对括号的闭合层级; 不确定时优先简化结构, 不要凑数。
