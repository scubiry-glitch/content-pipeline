// EventBusAdapter — 本地 EventEmitter 实现（嵌入模式）
// 复制自 content-library/adapters/local-event-bus.ts
  ...
  subscribe(event: string, handler: (payload: any) => Promise<void>): void {
    this.emitter.on(event, (payload) => {
      handler(payload).catch(err => {
        console.error(`[MeetingNotes/LocalEventBus] Error handling event ${event}:`, err);
      });
    });
  }
