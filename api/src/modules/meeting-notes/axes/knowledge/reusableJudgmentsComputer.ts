...
export async function computeReusableJudgments(
  ...
  for (const item of items) {
    try {
      const authorId = item.author ? (args.personRoster ? await args.personRoster.resolveAsync(item.author) : await ensurePersonByName(deps, item.author, undefined, undefined, args.meetingId)) : null;
