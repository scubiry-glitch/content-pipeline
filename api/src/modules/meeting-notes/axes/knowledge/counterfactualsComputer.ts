...
export async function computeCounterfactuals(
  ...
  if (args.replaceExisting) {
    await deps.db.query(
      // P0 数据源契约：只删 LLM 行
      `DELETE FROM mn_counterfactuals WHERE meeting_id = $1 AND source = 'llm_extracted'`,
      [bundle.meetingId],
    );
  }
  ...
  for (const item of items) {
    try {
      const rejectedById = item.rejected_by ? (args.personRoster ? await args.personRoster.resolveAsync(item.rejected_by) : await ensurePersonByName(deps, item.rejected_by, undefined, undefined, args.meetingId)) : null;
