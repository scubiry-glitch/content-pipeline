...
async function setupMVPSchema(): Promise<void> {
  ...
  // v7.6 会议纪要采集渠道 (migration 028) —— 与 rss_sources 同类的上游采集配置
  await query(`
  ...
  `);
  await query(`CREATE INDEX IF NOT EXISTS idx_meeting_note_sources_active ON meeting_note_sources(is_active, kind)`);
