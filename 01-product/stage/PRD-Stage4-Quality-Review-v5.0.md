...
async function syncToBlueTeamReviews(taskId: string, expertReviews: any[]): Promise<void> {
  // 1. 清理该任务在 blue_team_reviews 中的旧数据
...
{task?.status === 'awaiting_approval' && 
 ...
 item.severity !== 'praise' && (
  <div className="review-actions">
    <button onClick={() => onReviewDecision(item.reviewId, item.id, 'accept')}>
      ✓ 接受修改
    </button>
    <button onClick={() => onReviewDecision(item.reviewId, item.id, 'manual_resolved')}>
      ✓ 已手动处理
    </button>
    <button onClick={() => onReviewDecision(item.reviewId, item.id, 'ignore')}>
      ⊘ 忽略
    </button>
  </div>
)}
