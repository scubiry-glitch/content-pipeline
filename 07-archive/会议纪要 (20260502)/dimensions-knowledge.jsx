...
function DimensionKnowledge() {
  ...
  const tabs = [
    ...
    { id:"consensus",      label:"共识与分歧", sub:"跨会议追踪 · 稳定共识 vs 持续分歧", icon:"users" },
    { id:"concept",        label:"概念辨析",   sub:"同词异义诊断", icon:"wand" },
    { id:"lineage",        label:"议题谱系",   sub:"出生 / 健康 / 濒危议题", icon:"git" },
    { id:"external",       label:"外脑批注",   sub:"图书馆专家在边上写的批注", icon:"book" },
  ];
  return (
    <window.DimShell axis="知识" tabs={tabs} tab={tab} setTab={setTab}>
      ...
      {tab==="consensus"       && <window.KConsensusDivergence/>}
      {tab==="concept"         && <window.KConceptDrift/>}
      {tab==="lineage"         && <window.KTopicLineage/>}
      {tab==="external"        && <window.KExternalAnnotations/>}
    </window.DimShell>
  );
}
