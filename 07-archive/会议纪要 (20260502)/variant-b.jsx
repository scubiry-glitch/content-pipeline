// variant-b.jsx — Workbench direction
// 密集型三栏工作台：左侧导航+专家，主栏按维度切换，右栏 source-of-truth 原文

function VariantWorkbench() {
  const [dim, setDim] = React.useState("tension");
  const [selectedT, setSelectedT] = React.useState("T1");
  const a = window.ANALYSIS;

  const dimList = [
    { id:"minutes",       icon:"ledger",  label:"常规纪要" },
    { id:"tension",       icon:"bolt",    label:"张力" },
    { id:"new_cognition", icon:"sparkle", label:"新认知" },
    { id:"focus_map",     icon:"target",  label:"关注点" },
    { id:"consensus",     icon:"scale",   label:"共识/分歧" },
    { id:"cross_view",    icon:"network", label:"观点对位" },
  ];
    ...
    <div style={{
      ...
      <header style={{
        ...
        <div style={{display:"flex", alignItems:"center", gap:8}}>
          ...
          <window.MonoMeta>/ workbench</window.MonoMeta>
        </div>
        <div style={{
          height:22, width:1, background:"var(--line)",
        }}/>
        <div style={{display:"flex", alignItems:"center", gap:6, fontSize:12, color:"var(--ink-2)"}}>
          <window.Icon name="folder" size={14}/>
          <span>{window.MEETING.id}</span>
          <window.Icon name="chevron" size={12} style={{color:"var(--ink-4)"}}/>
          <span style={{fontWeight:500, color:"var(--ink)"}}>{window.MEETING.title}</span>
        </div>
        <div style={{marginLeft:"auto", display:"flex", gap:6, alignItems:"center"}}>
          <window.Chip tone="ghost"><window.Icon name="mic" size={11}/>m4a + docx</window.Chip>
          <window.Chip tone="ghost">preset: standard</window.Chip>
          <window.Chip tone="accent">3 experts active</window.Chip>
      ...
      <div style={{display:"grid", gridTemplateColumns:"220px 1fr 360px", height:"100%", minHeight:0}}>
        ...
        <aside style={{
          ...
          <div style={{padding:"18px 16px 10px"}}>
            <window.SectionLabel>维度 Dimensions</window.SectionLabel>
          </div>
            ...
            {dimList.map(d=>{
              ...
              return (
                <button key={d.id} onClick={()=>setDim(d.id)} style={{
                  ...
                  <window.Icon name={d.icon} size={15}/>
                  <span>{d.label}</span>
                  {active && <window.Icon name="dot" size={8} style={{marginLeft:"auto", color:"var(--accent)"}}/>}
                </button>
              );
            })}
          </nav>
          ...
          <div style={{padding:"22px 16px 10px"}}>
            <window.SectionLabel>专家栈 Experts</window.SectionLabel>
          </div>
          <div style={{padding:"0 10px", display:"flex", flexDirection:"column", gap:6, overflowY:"auto"}}>
            {window.EXPERTS.filter(e=>e.selected).map(e=>(
              <div key={e.id} style={{
                ...
                <div style={{display:"flex", alignItems:"center", gap:8, justifyContent:"space-between"}}>
                  ...
                  <window.Chip tone="ink" style={{padding:"1px 6px", fontSize:10}}>{(e.match*100).toFixed(0)}%</window.Chip>
                </div>
            ...
            <button style={{
              ...
              <window.Icon name="plus" size={12}/> 添加专家
            </button>
          </div>
        </aside>
        ...
        <section style={{overflow:"hidden", display:"flex", flexDirection:"column", minWidth:0}}>
          <div style={{
            ...
            }}>{dimList.find(x=>x.id===dim).label}</h2>
            <window.MonoMeta>
            ...
            <div style={{marginLeft:"auto", display:"flex", gap:6}}>
              <window.Chip tone="ghost"><window.Icon name="sparkle" size={10}/>知识锚定已启用</window.Chip>
              <window.Chip tone="ghost">置信度: 0.78</window.Chip>
            </div>
          </div>
        ...
        <aside style={{
          ...
          <div style={{padding:"14px 16px", borderBottom:"1px solid var(--line)"}}>
            <window.SectionLabel>原文锚点 Evidence</window.SectionLabel>
            ...
            {mockTranscript.map((t,i)=>(
              <div key={i} style={{
                ...
                <div style={{display:"flex", alignItems:"center", gap:6, marginBottom:3}}>
                  <window.Avatar p={window.P(t.who)} size={16} radius={3} />
                  <span style={{fontSize:11, fontWeight:600, color:"var(--ink)"}}>{window.P(t.who).name}</span>
                  <window.MonoMeta style={{marginLeft:"auto", fontSize:10}}>{t.time}</window.MonoMeta>
                </div>
                <div style={{
                  fontSize:12.5, lineHeight:1.6, color: t.highlight ? "var(--ink)" : "var(--ink-2)",
                  fontFamily:"var(--serif)",
                }}>{t.text}</div>
                {t.tag && <div style={{marginTop:6, display:"flex", gap:4, flexWrap:"wrap"}}>
                  {t.tag.map((x,j)=><window.Chip key={j} tone="accent" style={{fontSize:10, padding:"1px 6px"}}>{x}</window.Chip>)}
                </div>}
              </div>
            ))}
          </div>
...
const TopBtn = ({ icon, children }) => (
  <button style={{
    display:"flex", alignItems:"center", gap:6, padding:"5px 10px", border:"1px solid var(--line)",
    background:"var(--paper)", color:"var(--ink-2)", fontSize:12, borderRadius:5, cursor:"pointer", fontFamily:"var(--sans)",
  }}>
    <window.Icon name={icon} size={12}/> {children}
  </button>
);

// Small curated transcript for the right pane
const mockTranscript = [
  ...
  { who:"p1", time:"00:54:22", text:"上限可以谈，但 8000 万那种单笔，我们要准备好跟 LP 沟通预案。", tag:["D1"] },
  { who:"p5", time:"00:55:03", text:"我只提醒，决策还是你们。", tag:["D1"] },
  { who:"p6", time:"01:12:30", text:"过去 6 个月我们这边提供了 18 个 warm intro，有 4 个进到 term sheet。", tag:["N2","V2"] },
  { who:"p4", time:"01:25:58", text:"同类样本历史中位数 38%，我倾向于拆 cohort 再看。", tag:["V1"] },
];
  ...
  return (
    <div style={{display:"grid", gridTemplateColumns:"260px 1fr", gap:16, height:"100%"}}>
      <div style={{display:"flex", flexDirection:"column", gap:6}}>
        {a.tension.map(t=>{
          const [p1,p2] = t.between.map(window.P);
            ...
            <button key={t.id} onClick={()=>setSelected(t.id)} style={{
              ...
              <div style={{display:"flex", justifyContent:"space-between", alignItems:"center"}}>
                ...
                <div style={{display:"flex", gap:3, alignItems:"center"}}>
                  ...
                  <window.MonoMeta style={{fontSize:10}}>{(t.intensity*100).toFixed(0)}</window.MonoMeta>
                </div>
              </div>
              ...
              <div style={{display:"flex", alignItems:"center", gap:4, fontSize:11, color:"var(--ink-3)"}}>
                <window.Avatar p={p1} size={16} radius={3}/>
                <span>{p1.name}</span>
                <span style={{color:"var(--ink-4)"}}>↔</span>
                <window.Avatar p={p2} size={16} radius={3}/>
                <span>{p2.name}</span>
              </div>
            </button>
          );
      ...
      <div style={{
        border:"1px solid var(--line-2)", borderRadius:8, padding:"20px 22px", background:"var(--paper)",
        overflow:"auto",
      }}>
        {(() => {
          const t = a.tension.find(x=>x.id===selected);
          const [p1,p2] = t.between.map(window.P);
          return (
            <>
              <div style={{display:"flex", alignItems:"center", gap:12, marginBottom:12}}>
                <window.MonoMeta>{t.id}</window.MonoMeta>
                <window.Chip tone="accent">强度 {(t.intensity*100).toFixed(0)}</window.Chip>
              </div>
              <h3 style={{fontFamily:"var(--serif)", fontSize:22, margin:"0 0 12px", fontWeight:600, letterSpacing:"-0.01em"}}>
                {t.topic}
              </h3>

              <div style={{
                display:"grid", gridTemplateColumns:"1fr auto 1fr", gap:14, alignItems:"stretch", marginBottom:20,
              }}>
                <Stance p={p1} stance="沈岚立场" text="推理层毛利更耐周期；中游可防御。" tone="accent"/>
                ...
                <Stance p={p2} stance="Wei Tan 立场" text="训练层规模效应才是真正护城河。" tone="teal"/>
              </div>

              <p style={{fontSize:14, lineHeight:1.65, color:"var(--ink-2)", fontFamily:"var(--serif)"}}>{t.summary}</p>

              {t.moments.length>0 && (
                <div style={{marginTop:18, display:"flex", flexDirection:"column", gap:6}}>
                  <window.SectionLabel>关键段落</window.SectionLabel>
                  {t.moments.map((m,i)=>(
                    <div key={i} style={{
              ...
              <div style={{
                ...
                <window.SectionLabel>专家解读 · E09-09</window.SectionLabel>
                <p style={{fontSize:13.5, lineHeight:1.7, margin:"8px 0 0", color:"var(--ink-2)"}}>
                  从二阶效应看，这里的真正问题不是"哪一层更好"，而是一个组合敞口问题。若推理层胜出，我们的头部布局已
                  足够；若训练层胜出，现有仓位会被摊薄稀释。当前分歧其实是在 hedge 的粒度上没有对齐。<window.Chip tone="ghost" style={{marginLeft:6}}>[M#42]</window.Chip>
                </p>
              </div>
            </>
          );
...
function WBMinutes({ a }) {
  return (
    <div style={{display:"flex", flexDirection:"column", gap:16}}>
      <div style={{
        padding:"16px 20px", background:"var(--paper)", border:"1px solid var(--line-2)", borderRadius:6,
      }}>
        <window.SectionLabel>Decision</window.SectionLabel>
        <div style={{fontFamily:"var(--serif)", fontSize:17, lineHeight:1.55, marginTop:6}}>{a.summary.decision}</div>
      </div>
      <div style={{
        padding:"4px 0", background:"var(--paper)", border:"1px solid var(--line-2)", borderRadius:6,
      }}>
        <div style={{padding:"12px 20px", borderBottom:"1px solid var(--line-2)"}}>
          <window.SectionLabel>Action items · {a.summary.actionItems.length}</window.SectionLabel>
        </div>
        {a.summary.actionItems.map((it,idx)=>(
          <div key={it.id} style={{
            ...
            <window.MonoMeta>{it.due}</window.MonoMeta>
      ...
      <div style={{padding:"16px 20px", background:"var(--paper)", border:"1px solid var(--line-2)", borderRadius:6}}>
        <window.SectionLabel>Open risks · {a.summary.risks.length}</window.SectionLabel>
        ...
        return (
          <div key={n.id} style={{
            ...
            <div style={{display:"flex", alignItems:"center", gap:10, marginBottom:10}}>
              <window.Avatar p={p} size={24}/>
              <span style={{fontSize:13, fontWeight:500}}>{p.name}</span>
              <window.MonoMeta>{n.id}</window.MonoMeta>
              <window.Chip tone="teal" style={{marginLeft:"auto"}}>信念更新</window.Chip>
            </div>
            <div style={{
              display:"grid", gridTemplateColumns:"1fr 32px 1fr", gap:10, alignItems:"stretch",
            }}>
              <div style={{
                background:"var(--paper-2)", padding:"10px 12px", borderRadius:4, fontSize:13, lineHeight:1.5,
              ...
              <div style={{display:"flex", alignItems:"center", justifyContent:"center", color:"var(--teal)"}}>
                <window.Icon name="arrow" size={20}/>
              </div>
              <div style={{
                background:"var(--teal-soft)", padding:"10px 12px", borderRadius:4, fontSize:13, lineHeight:1.5,
                color:"oklch(0.28 0.08 200)", fontFamily:"var(--serif)", fontWeight:500,
              }}>{n.after}</div>
            </div>
    ...
    <div style={{display:"grid", gridTemplateColumns:"1fr 1fr", gap:14}}>
      <div style={{background:"var(--paper)", border:"1px solid var(--line-2)", borderRadius:6}}>
        <div style={{padding:"10px 14px", borderBottom:"1px solid var(--line-2)", display:"flex", alignItems:"center", gap:8}}>
          <window.Icon name="check" size={14} style={{color:"var(--accent)"}}/>
          ...
          <div key={c.id} style={{padding:"12px 14px", borderBottom:"1px solid var(--line-2)"}}>
            ...
            <div style={{display:"flex", gap:4, marginTop:8}}>
              {c.supportedBy.map(id=>(
                <window.Avatar key={id} p={window.P(id)} size={18} radius={4}/>
              ))}
              <window.MonoMeta style={{marginLeft:6, alignSelf:"center"}}>{c.supportedBy.length}/6 支持</window.MonoMeta>
            </div>
          </div>
        ))}
      </div>
      <div style={{background:"var(--paper)", border:"1px solid var(--line-2)", borderRadius:6}}>
        <div style={{padding:"10px 14px", borderBottom:"1px solid var(--line-2)", display:"flex", alignItems:"center", gap:8}}>
          <window.Icon name="git" size={14} style={{color:"var(--teal)"}}/>
            ...
            {d.sides.map((s,i)=>(
              <div key={i} style={{
                ...
                <div style={{display:"flex", gap:2}}>
                  {s.by.map(id=><window.Avatar key={id} p={window.P(id)} size={16} radius={3}/>)}
                </div>
              </div>
            ))}
          </div>
...
function WBCrossView({ a }) {
  const tones = { support:"accent", oppose:"teal", partial:"amber", neutral:"ghost" };
  const labels = { support:"附议", oppose:"反对", partial:"部分认同", neutral:"中性" };
  return (
    <div style={{display:"flex", flexDirection:"column", gap:18}}>
      {a.crossView.map(v=>{
        const p = window.P(v.claimBy);
        return (
          <div key={v.id} style={{background:"var(--paper)", border:"1px solid var(--line-2)", borderRadius:6, overflow:"hidden"}}>
            <div style={{padding:"14px 16px", borderBottom:"1px solid var(--line-2)", background:"var(--paper-2)"}}>
              <div style={{display:"flex", alignItems:"center", gap:8, marginBottom:6}}>
                ...
                <window.MonoMeta style={{marginLeft:"auto"}}>{v.id}</window.MonoMeta>
              </div>
              <div style={{fontFamily:"var(--serif)", fontSize:16, fontWeight:500, lineHeight:1.45}}>{v.claim}</div>
            </div>
            <div>
              {v.responses.map((r,i)=>{
                const rp = window.P(r.who);
                return (
                  <div key={i} style={{
                    ...
                    <div style={{display:"flex", alignItems:"center", gap:6}}>
                      <window.Avatar p={rp} size={18} radius={4}/>
                      <span style={{fontSize:12}}>{rp.name}</span>
                    </div>
                    <window.Chip tone={tones[r.stance]}>{labels[r.stance]}</window.Chip>
