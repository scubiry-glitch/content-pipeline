// scope-expert-config.jsx — 专家调用系统的 scope 作用层切换
// 同一套专家，三种装配方式：meeting / project / library

// ─────────────────────────────────────────────────────────
// Scope-specific strategies (扩展 §6.3)
// ─────────────────────────────────────────────────────────
...
// ─────────────────────────────────────────────────────────
// Scope-specific decorators (扩展 §6.4)
// ─────────────────────────────────────────────────────────
...
// Main component
// ─────────────────────────────────────────────────────────
function ScopeExpertConfig() {
  const [scope, setScope] = React.useState("meeting"); // meeting | project | library
  const [preset, setPreset] = React.useState("standard");

  const scopes = [
    { id:"meeting",  label:"会议层",  kind:"PER-MEETING",  sub:"单场会议 · 产出 6 维解析",      input:"原始素材",       output:"6 维解析纪要" },
    { id:"project",  label:"项目层",  kind:"PER-PROJECT",  sub:"项目内跨会议 · 纵向演化",        input:"N 场已解析纪要", output:"项目纵向报告" },
    { id:"library",  label:"库层",    kind:"PER-LIBRARY",  sub:"全库跨项目 · 团队级偏误审计",    input:"全库纪要 + 专家校准表", output:"团队认知报告" },
  ];
    ...
    <div style={{
      ...
      <header style={{padding:"26px 36px 18px", borderBottom:"1px solid var(--line-2)"}}>
        <div style={{display:"flex", alignItems:"baseline", gap:14}}>
          ...
          <window.MonoMeta>presets.scope.ts</window.MonoMeta>
        </div>
        ...
        <div style={{
          ...
          {scopes.map(s=>{
            ...
            return (
              <button key={s.id} onClick={()=>{setScope(s.id); setPreset("standard");}}
                style={{
                ...
                <div style={{fontSize:12, opacity: active?.85:.7, lineHeight:1.4}}>{s.sub}</div>
                <div style={{
                  fontFamily:"var(--mono)", fontSize:10.5, marginTop:4,
                  color: active ? "rgba(255,255,255,.7)" : "var(--ink-3)",
                }}>
      ...
      <div style={{flex:1, overflow:"auto", padding:"22px 36px 30px"}}>
        ...
        <div style={{marginBottom:24}}>
          <window.SectionLabel>Presets · {scopeMeta.label}的 3 档</window.SectionLabel>
          <div style={{
              ...
              return (
                <div key={p.id} onClick={()=>setPreset(p.id)} style={{
                  ...
                  <div style={{display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:8}}>
                    ...
                    {p.default && <window.Chip tone="accent">default</window.Chip>}
                  </div>
        ...
        {/* Two-column: strategies + decorators for this scope */}
        <div style={{display:"grid", gridTemplateColumns:"1fr 1fr", gap:22}}>
          {/* Strategies */}
          <section>
            <window.SectionLabel>可用策略 · {strategies.length}</window.SectionLabel>
                  ...
                  <div key={s.id} style={{
                    ...
                    <div style={{flex:1}}>
                      <div style={{display:"flex", alignItems:"center", gap:8}}>
                        ...
                        {inUse && <window.Chip tone="accent" style={{padding:"1px 6px", fontSize:9.5}}>in use</window.Chip>}
                      </div>
          ...
          {/* Decorators */}
          <section>
            <window.SectionLabel>可用装饰器 · {decorators.length}</window.SectionLabel>
                  ...
                  <div key={d.id} style={{
                    ...
                    {inUse && (
                      <div style={{
                        ...
                        <window.Icon name="check" size={9} stroke={3} style={{color:"var(--paper)"}}/>
                      </div>
                    )}
                  </div>
                );
        ...
        {/* Bottom: cost comparison across scopes */}
        <div style={{marginTop:26}}>
          <window.SectionLabel>跨 scope 的调用频率与成本预算</window.SectionLabel>
          <div style={{
...
Object.assign(window, { ScopeExpertConfig });
