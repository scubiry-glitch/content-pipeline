...
function DimensionProjects() {
  ...
  const tabs = [
    ...
    { id:"accountability", label:"责任盘点", sub:"谁在推 · 谁在卡 · 谁旁观",   icon:"users" },
    { id:"externalimpact", label:"对外影响", sub:"决议如何投射到外部世界",     icon:"send" },
  ];
  return (
    <window.DimShell axis="项目" tabs={tabs} tab={tab} setTab={setTab}>
      ...
      {tab==="accountability" && <window.PJAccountability/>}
      {tab==="externalimpact" && <window.PJExternalImpact/>}
    </window.DimShell>
  );
}
